...
🔧 relay-via minimal patch ⟦v2026.08.16-r4⟧
----------------------------------------------------------
...
function ResourceParse() {
  ...
  if (flag == 1) { //server 类型统一处理
    ...
    if (total.length > 0){
      ...
      if (PRelay!="" && PRelayVia!="") {
        total = total.split("\n").filter(Boolean).map(item => item + (PRelayVia=="0" ? ", via-interface=%TUN%" : ", via-interface="+PRelayVia)).join("\n")
      }
      if (PNS !=0) {
        if (version >913) {
...
function ServerRelay(src,dst) {
  ...
  for (var i=0; i<src.length; i++) {
    if (src[i].indexOf("-host")==-1 && /^\[[0-9a-fA-F:]+\]:/.test(src[i].split("=")[1].trim())) {
      serverA = src[i].split("=")[1].trim().split("]")[0].replace("[","")
      rsts.push("ip6-cidr,"+serverA+"/128,"+dst)
      continue
    }
